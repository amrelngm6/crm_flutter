...
class ProposalStatisticsCard extends StatelessWidget {
  final Map<String, dynamic> statistics;

  const ProposalStatisticsCard({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 30),
        _buildValueRanges(valueRanges),
        const SizedBox(height: 30),
        _buildClientBreakdown(clientBreakdown),
        const SizedBox(height: 30),
      ...
      children: [
        ...
        GridView.count(
          ...
          children: [
            _buildOverviewCard(
              'Total Proposals',
              '${overview['total_proposals'] ?? 0}',
              Icons.description,
              const Color(0xFF27ae60),
            ),
            _buildOverviewCard(
              'My Proposals',
              '${overview['my_proposals'] ?? 0}',
              Icons.person,
  ...
  Widget _buildValueRanges(List<dynamic> valueRanges) {
    if (valueRanges.isEmpty) return const SizedBox.shrink();
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            children: valueRanges.map<Widget>((range) {
              final rangeName = range['range'] ?? 'Unknown';
              final count = range['count'] ?? 0;
              final color = _getValueRangeColor(rangeName);
  ...
  Widget _buildClientBreakdown(List<dynamic> clientBreakdown) {
    if (clientBreakdown.isEmpty) return const SizedBox.shrink();
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            children: clientBreakdown.take(8).map<Widget>((client) {
              final clientName = client['client_name'] ?? 'Unknown';
              final count = client['count'] ?? 0;
              final total = client['total'] ?? 0.0;
              final color = _getClientColor(clientName);
        ...
        Container(
          ...
          child: Column(
            children: assignmentBreakdown.take(8).map<Widget>((assignment) {
              ...
              final total = assignment['total'] ?? 0.0;
  ...
  Color _getValueRangeColor(String rangeName) {
    if (rangeName.contains('0-1000')) return const Color(0xFF95a5a6);
    if (rangeName.contains('1000-5000')) return const Color(0xFF3498db);
    if (rangeName.contains('5000-10000')) return const Color(0xFFf39c12);
    if (rangeName.contains('10000-50000')) return const Color(0xFF9b59b6);
    if (rangeName.contains('50000+')) return const Color(0xFFe74c3c);
    return const Color(0xFF6c757d);
  }

  Color _getClientColor(String clientName) {
    // Generate consistent colors for different clients
    final colors = [
      const Color(0xFF3498db),
      const Color(0xFF9b59b6),
      const Color(0xFFe74c3c),
    ...
    final index = clientName.hashCode % colors.length;
